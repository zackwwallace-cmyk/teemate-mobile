...
async function getUnavailablePartnerIds(userId: string) { const unavailable = new Set<string>([userId]); const { data: existing, error: matchError } = await supabase.from('matches').select('golfer_a,golfer_b,status').or(`golfer_a.eq.${userId},golfer_b.eq.${userId}`); if (matchError) return { ids: unavailable, error: matchError }; existing?.forEach((match: any) => { unavailable.add(match.golfer_a === userId ? match.golfer_b : match.golfer_a); }); const { data: blocks, error: blockError } = await supabase.from('blocked_users').select('blocker_id,blocked_id').or(`blocker_id.eq.${userId},blocked_id.eq.${userId}`); if (blockError) return { ids: unavailable, error: blockError }; blocks?.forEach((block: any) => { if (block.blocker_id === userId) unavailable.add(block.blocked_id); if (block.blocked_id === userId) unavailable.add(block.blocker_id); }); return { ids: unavailable, error: null }; }
export async function getDiscoverStack(userId: string) { const { data: myProfile } = await getMyProfile(userId); const unavailable = await getUnavailablePartnerIds(userId); if (unavailable.error) return { data: null, error: unavailable.error }; const { data: myCourses } = await supabase.from('profile_courses').select('course_id').eq('profile_id', userId); const myCourseIds = myCourses?.map((row: any) => row.course_id) ?? []; const { data: others, error } = await fetchCandidateProfiles(); if (error) return { data: null, error }; const candidates = (others ?? []).filter((profile) => !unavailable.ids.has(profile.id)); const ids = candidates.map((profile) => profile.id); const { data: profileCourses } = ids.length ? await supabase.from('profile_courses').select('profile_id,course_id').in('profile_id', ids) : { data: [] as any[] }; const courseMap = new Map<string, string[]>(); profileCourses?.forEach((row: any) => { const existingCourses = courseMap.get(row.profile_id) ?? []; existingCourses.push(row.course_id); courseMap.set(row.profile_id, existingCourses); }); const stack = candidates.map((profile) => { const withCourses = { ...profile, course_ids: courseMap.get(profile.id) ?? [] }; return { ...withCourses, score: scoreProfile(myProfile as Profile | null, withCourses, myCourseIds) }; }).sort((a, b) => (b.score ?? 0) - (a.score ?? 0)); return { data: stack, error: null }; }
export async function getDiscoverProfiles(userId: string) { return getDiscoverStack(userId); }
export async function createOrUpdateMatch(currentUserId: string, otherUserId: string, status: 'pending' | 'declined' | 'matched', matchScore?: number | null) {
  const existing = await supabase.from('matches').select('*').or(`and(golfer_a.eq.${currentUserId},golfer_b.eq.${otherUserId}),and(golfer_a.eq.${otherUserId},golfer_b.eq.${currentUserId})`).maybeSingle<Match>();
  if (existing.error) return { data: null, error: existing.error };
  if (existing.data) {
    if (existing.data.status === 'declined' && status === 'pending') return { data: existing.data, error: null };
    const reciprocalPending = status === 'pending' && existing.data.status === 'pending' && existing.data.initiated_by !== currentUserId;
    const nextStatus = reciprocalPending ? 'matched' : status;
    const nextInitiatedBy = nextStatus === 'matched' ? existing.data.initiated_by : currentUserId;
    const result = await supabase.from('matches').update({ status: nextStatus, initiated_by: nextInitiatedBy, match_score: matchScore ?? existing.data.match_score ?? null, updated_at: new Date().toISOString() }).eq('id', existing.data.id).select('*').single<Match>();
    if (!result.error && result.data) {
      if (reciprocalPending || (status === 'matched' && existing.data.status !== 'matched')) await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'match', matchId: result.data.id });
      else if (status === 'pending' && existing.data.status !== 'pending') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'connection_request', matchId: result.data.id });
    }
    return result;
  }
  const result = await supabase.from('matches').insert({ golfer_a: currentUserId, golfer_b: otherUserId, initiated_by: currentUserId, status, match_score: matchScore ?? null }).select('*').single<Match>();
  if (!result.error && result.data) {
    if (status === 'pending') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'connection_request', matchId: result.data.id });
    if (status === 'matched') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'match', matchId: result.data.id });
  }
  return result;
}
...
export async function updateMatchStatus(matchId: string, status: 'matched' | 'declined' | 'canceled') { const result = await supabase.from('matches').update({ status, updated_at: new Date().toISOString() }).eq('id', matchId).select('*').single<Match>(); if (!result.error && result.data && status === 'matched') { await safeSendConnectionPush({ recipientIds: [result.data.golfer_a, result.data.golfer_b], actorId: result.data.initiated_by, type: 'match', matchId: result.data.id }); } return result; }
...
export async function updateFeedPost(postId: string, userId: string, body: string) { return supabase.from('posts').update({ body }).eq('postId', postId).eq('author_id', userId).select('*').single(); }
