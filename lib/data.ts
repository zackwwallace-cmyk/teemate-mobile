...
export async function createOrUpdateMatch(currentUserId: string, otherUserId: string, status: 'pending' | 'declined' | 'matched', matchScore?: number | null) { const existing = await supabase.from('matches').select('*').or(`and(golfer_a.eq.${currentUserId},golfer_b.eq.${otherUserId}),and(golfer_a.eq.${otherUserId},golfer_b.eq.${currentUserId})`).maybeSingle<Match>(); if (existing.error) return { data: null, error: existing.error }; if (existing.data) { if (existing.data.status === 'declined' && status === 'pending') return { data: existing.data, error: null }; const reciprocalPending = status === 'pending' && existing.data.status === 'pending' && existing.data.initiated_by !== currentUserId; const nextStatus = reciprocalPending ? 'matched' : status; const nextInitiatedBy = nextStatus === 'matched' ? existing.data.initiated_by : currentUserId; const result = await supabase.from('matches').update({ status: nextStatus, initiated_by: nextInitiatedBy, match_score: matchScore ?? existing.data.match_score ?? null, updated_at: new Date().toISOString() }).eq('id', existing.data.id).select('*').single<Match>(); if (!result.error && result.data) { if (reciprocalPending || (status === 'matched' && existing.data.status !== 'matched')) await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'match', matchId: result.data.id }); else if (status === 'pending' && existing.data.status !== 'pending') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'connection_request', matchId: result.data.id }); } return result; } const result = await supabase.from('matches').insert({ golfer_a: currentUserId, golfer_b: otherUserId, initiated_by: currentUserId, status, match_score: matchScore ?? null }).select('*').single<Match>(); if (!result.error && result.data) { if (status === 'pending') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'connection_request', matchId: result.data.id }); if (status === 'matched') await safeSendConnectionPush({ recipientIds: [otherUserId], actorId: currentUserId, type: 'match', matchId: result.data.id }); } return result; }
