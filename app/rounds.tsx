...
function RoundCard(props: any) {
  const { round, isHost, approved, pending, full, confirmedPlayers, pendingRequests, joining, busy, onOpenChat, onRequestJoin, onLeave, onApprove, onDecline, onComplete, onCancel } = props;
  const openSlots = Math.max(0, round.open_slots ?? 0);
  return (
    <View style={styles.card}>
      <TouchableOpacity disabled={!approved} onPress={onOpenChat} style={styles.cardHeader}>
        <Ionicons name="flag-outline" size={20} color={colors.pine} />
        <View style={styles.flex}>
          <Text style={styles.course}>{round.course_text || 'Golf round'}</Text>
          <Text style={styles.meta}>{round.town || 'Local course'} • {prettyRound(round)}</Text>
        </View>
        {approved ? <Ionicons name="chatbubbles-outline" size={20} color={colors.pine} /> : null}
      </TouchableOpacity>
